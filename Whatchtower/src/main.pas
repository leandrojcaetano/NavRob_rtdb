...
//------------------------------------------------------------------------------
//
// Programa de Supervisão para times de Robôs Móveis
// A princípio utilizado para o TurtleBot 2
// Na segunda fase para: Drone Carcará e Hexapod Minion
//
// FALTA FAZER:
//             - COMUNICAÇÃO COM RoC POR RTDB           (Leandro)
//             - SUPERVISÃO DE VÁRIOS TIMES DE ROBÔS    (Mestrado 2)
//             - QUEBRA DE FORMAÇÃO INTELIGENTE         (Mestrado 2)
...
uses Field, Robots, Utils, Tatic, Param, Log;
...
procedure TFMain.WorldToMapDim(dw: double; var dm: integer);
begin
  dm:=round(dw*Dfieldmag);
end;

procedure TFMain.WorldToMap(xw, yw: double; var xm: integer; var ym: integer);
begin
  xm:=round(FieldImageWidth/2+xw*Dfieldmag);
  ym:=round(FieldImageHeight/2-yw*Dfieldmag);
end;

function TFMain.WorldToMapP(xw, yw: double): TPoint;
begin
  result.x:=round(FieldImageWidth/2+xw*Dfieldmag);
  result.y:=round(FieldImageHeight/2-yw*Dfieldmag)
end;

procedure TFMain.MapToWorld(xm, ym: integer; var xw, yw: double);
begin
  xw:=(xm-FieldImageWidth/2)/Dfieldmag;
  yw:=(FieldImageHeight/2-ym)/Dfieldmag;
end;

function TFMain.Get24Color(cor: integer): Tcolor;
