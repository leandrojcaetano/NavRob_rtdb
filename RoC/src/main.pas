...
uses
  ...
  math, dynmatrix, IniFiles, CheckLst, ComCtrls, Grids, Roles, rtdb_api;
...
type
  ...
  TFMain = class(TForm)
    ...
    procedure ImageMapMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure MemoActionParsChange(Sender: TObject);
 ...
 //   procedure SendPlayerInfo;
  ...
  public
    ...
    // POR LEANDRO
   // procedure RetreiveDataFromRTDB_should_be_deleted();
...
// POR LEANDRO
 if(not (rtdb_api.db_init(myNumber)=0) ) then   // SABER ONDE É SETADO A VARIÁVEL QUE POSSUI A ID DO ROBO?
  begin
...
procedure TFMain.MemoActionParsChange(Sender: TObject);
begin
...
begin
     ...
     rtdb_api.DB_put(myNumber, @temp);
end;

procedure TFMain.Button2Click(Sender: TObject);
var temp2,temp: integer;
begin
 // StrToInt(GetEnvironmentVariable("AGENT"));
    temp2 := rtdb_api.DB_get(StrToInt(GetEnvironmentVariable('AGENT')),temp2, @temp);
    Label3.Caption:= IntToStr(temp);
end;

procedure TFMain.CBSimChange(Sender: TObject);
var teste: integer;
begin
 //    teste:= 2;
//  rtdb_api.DB_put(myNumber, @teste);
end;
...
//  SendPlayerInfo;
 ...
 {
procedure TFMain.RetreiveDataFromRTDB_should_be_deleted();
var ball_status: integer;
    robot_info: integer;
    i,age: integer;
begin

     // update ONE robot state
     for i:=Robot1 to Robot5 do
     begin
          with RobotState[i-Robot1] do
          begin
               case i of
     ...
     for i:=Robot1 to Robot5 do
     begin
          write('Data age['); write(i);write('] = ');
          age:=rtdb_api.DB_get(i, ROB_INFO, @robot_info);
          if ((age>=0)and(RobotStateForm[i-Robot1].CBRobotActive.Checked)) then
          begin
            write('Robot['); write(i);write('].x = ');
            writeln(robot_info.x);
            with RobotState[i-Robot1] do
            begin
                 x:=robot_info.x;
                 y:=robot_info.y;
            ...
            with RobotStatus[i-Robot1] do
            begin
                if RobotStateForm[i-Robot1].CBForceRobotActive.Checked then
                begin
                     Active:=RobotStateForm[i-Robot1].CBRobotActive.Checked
                end
            else
                if (RobotStateForm[i-Robot1].CBRobotActive.Checked) and (robot_info.active=1)  then
                begin
                     active:=true;
                end
                else
  ...
  // update this robot's ball state for future merging
     ...
     begin
          ...
          //RTDB
          with BallStates[i-Robot1] do
          begin
               x:=ball_status.x;
               y:=ball_status.y;
               vx:=ball_status.vx;
               vy:=ball_status.vy;
               write('Ball robot');write(i);writeln(':');
               write('    x=');writeln(x);
               write('    y=');writeln(y);
               write('    vx=');writeln(vx);
               write('    vx=');writeln(vy);

               quality:=ball_status.quality;

          end;
     end;


         rtdb_api.DB_get(BaseStation, GLOBALBALL, @ball_status);
          //RTDB
               write('Ball Coach ');write(i);writeln(':');
               write('    x=');writeln(ball_status.x);
               write('    y=');writeln(ball_status.y);
               write('    vx=');writeln(ball_status.vx);
               write('    vx=');writeln(ball_status.vy);
